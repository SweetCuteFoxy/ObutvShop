...
public partial class FormOrders : Form
{
    ...
    public FormOrders(User? currentUser)
    {
        ...
        if (isAdmin)
            dataGridViewOrders.CellDoubleClick += (_, e) => { if (e.RowIndex >= 0) EditSelectedOrder(); };
    ...
    {
        using var form = new FormOrderEdit(null, _currentUser);
    ...
    private void EditSelectedOrder()
    {
        if (dataGridViewOrders.CurrentRow == null) return;
        if (!dataGridViewOrders.Columns.Contains("_OrderId")) return;
        if (dataGridViewOrders.CurrentRow.Cells["_OrderId"].Value is not int orderId) return;

        using var form = new FormOrderEdit(orderId, _currentUser);
        if (form.ShowDialog() == DialogResult.OK)
            LoadOrders();
    }
